...
HRESULT C3DSound::setPos(D3DVALUE fx, D3DVALUE fy, D3DVALUE fZ, DWORD dwApply)
{
    //DS3D_DEFERRED
    HRESULT hr;
    // Set the position of sound
    m_pDSB->SetVelocity(D3DVALUE(10.05f), D3DVALUE(0.05f), D3DVALUE(0.05f), DS3D_IMMEDIATE);
   
    if (FAILED(hr = m_pDSB->SetPosition(fx, fy, fZ, dwApply)))
        return hr;
